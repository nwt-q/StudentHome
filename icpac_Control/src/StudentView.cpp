...
/*
    ...
    返回值：void
*/
void WindowController::EasyTextBoxShow(EasyTextBox *NEW) {
    // 备份环境值
...
/*
    ...
    返回值：void
*/
EasyTextBox WindowController::EasyTextBoxCreate(int x1, int y1, int x2, int y2, int maxn) {   // 其中的x1, y1, x2, y2
...
/*
    ...
    返回值：bool
*/
bool WindowController::EasyTextBoxCheck(EasyTextBox* NEW, int x, int y) {
...
/*
    ...
    返回值：bool
*/
void WindowController::EasyTextBoxDelete(EasyTextBox* NEW) {
    if (NEW->text != NULL) {
        delete[] NEW->text;
    }
}
...
/*
    ...
    返回值：void
*/
void WindowController::EasyButtonShow(EasyButton* NEW)
{
    // 备份环境
    int oldlinecolor = getlinecolor();
    int oldbkcolor = getbkcolor();
...
/*
    ...
    返回值：void
*/
EasyButton WindowController::EasyButtonCreate(int x1, int y1, int x2, int y2, const char* title, void (*func)())
{
    EasyButton*NEW = new EasyButton(x1,y1,x2,y2,NULL,NULL);
    NEW->text = new char[strlen(title) + 1];
    std::string t(NEW->text);
    char tempBuffer[256]; // 假设256足够大
    strcpy_s(tempBuffer, sizeof(title) + 1, title);
    t = tempBuffer; // 将复制的字符串赋值给t
...
/*
    ...
    返回值：bool
*/
void WindowController::EasyButtonDelete(EasyButton* NEW)
{
    if (NEW->text != NULL)
        delete[] NEW->text;
}
...
/*
    ...
    返回值：bool
*/
bool WindowController::EasyButtonCheck(EasyButton* NEW ,int x, int y)
{
...
/*
    ...
    返回值：bool
*/
void WindowController::EasyButtonOnMessage(EasyButton* NEW)
{
...
/*
    ...
    返回值：bool
*/
int WindowController::On_btnOk_Click()
{
...
int WindowController::On_SendOk_Click()
{
    User* user = ReturnUser();
    int* Usernum = ReturnUserCount();
    Student NEW;
...
/*
    ...
    返回值：bool
*/
void WindowController::EasyTextBoxOnMessage(EasyTextBox* NEW)
{
    // 备份环境值
    int oldlinecolor = getlinecolor();
    int oldbkcolor = getbkcolor();
...
/*
    ...
    返回值：bool
*/
void WindowController::EasyPBoxOnMessage(EasyTextBox* NEW)
{
    // 备份环境值
    int oldlinecolor = getlinecolor();
    int oldbkcolor = getbkcolor();
...
/*
    ...
    返回值：void
*/
void WindowController::LoginView() {
    ...
    settextcolor(BLACK);
    btnOK =  EasyButton(620, 470, 700, 495, "Login", On_btnOk_Click());	// 创建按钮控件
    BtnOU =  EasyButton(420, 470, 500, 495, "OUT", WindowController::On_btnOk_Click());	// 创建按钮控件
...
/*
    ...
    返回值：void
*/
void WindowController::StudentSendView()
{
    using namespace gxb;
    IMAGE img;
    ...
    settextstyle(15, 0, "");//设置字号、字体
    BtnOU = EasyButton(420, 530, 500, 555, "OUT",FistPage());	// 创建按钮控件

    settextstyle(15, 0, "");//设置字号、字体
    btnOK = EasyButton(620, 530, 700, 555, "注册", On_SendOk_Click());	// 创建按钮控件
...
/*
    ...
    返回值：void
*/
void WindowController::StudentshowButton(int x, int y, int width, int height, std::string str, int textSize, Color fillColor, Color textColor)			//默认黑底，白字
{
...
/*
    ...
    返回值：void
*/
void WindowController::GameSelect() {
    using namespace gxb;
    cleardevice();
    IMAGE img;
...
/*
    ...
    返回值：void
*/
void WindowController::PersonalInformation() {
    using namespace gxb;
    cleardevice();
...
/*
    ...
    返回值：void
*/
void WindowController::StudentmenuView() {
    using namespace gxb;
    cleardevice();
    IMAGE img;
...
//学生设置页面
void WindowController::StudentSettingView() {
    using namespace gxb;
    cleardevice();
    IMAGE img;
...
void WindowController::StopView() {
    using namespace gxb;
    IMAGE img;
    ...
    StudentshowButton(600, 40, 100, 50, "惊喜按钮", 20, { 0,255,255 }, { 255,255,255 });
    ...
    while (true) {
        ...
        while (peekmessage(&msg, EM_MOUSE))
        {
            ...
            if (msg.message == WM_LBUTTONDOWN && inArea(msg.x, msg.y, 600, 40, 100, 50))
            {
...
void WindowController::StudentPage()
{
    using namespace gxb;
    IMAGE img;
    loadimage(&img, "../Photo/I.jpg", 1280, 720);/*变量地址，图片地址    相对地址“./”本目录下的文件进行访问   图片展示可以是png也可以是jpg*/
...
// 应用第一页
int WindowController::FistPage()
{
    using namespace gxb;
    IMAGE img;
    loadimage(&img, "../resources/Photo/I.jpg", 1280, 720);/*变量地址，图片地址    相对地址“./”本目录下的文件进行访问   图片展示可以是png也可以是jpg*/
    putimage(0, 0, &img);/*展示图片*/
    setbkmode(TRANSPARENT); // 用于解决黑色背景图
    settextcolor(WHITE);
    settextstyle(60, 0, _T("华文新魏"));//设置字号、字体
...
void WindowController::TimeViewFPS(IMAGE &img) {
    BeginBatchDraw();
    putimage(0, 0, &img);/*展示图片*/
...
// 学生计时专注页面
void WindowController::TimeView() {
    using namespace gxb;
    IMAGE img;
    cleardevice();
    loadimage(&img, "../Photo/I.jpg", 1280, 720);/*变量地址，图片地址    相对地址“./”本目录下的文件进行访问   图片展示可以是png也可以是jpg*/
